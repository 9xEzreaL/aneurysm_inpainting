...
class Upsample(nn.Module):
    """
    An upsampling layer with an optional convolution (3D version).
...
class Downsample(nn.Module):
    """
    A downsampling layer with an optional convolution (3D version).
    :param channels: channels in the inputs and outputs.
    :param use_conv: a bool determining if a convolution is applied.
    """
...
class ResBlock(EmbedBlock):
    """
    A residual block that can optionally change the number of channels (3D version).
...
class AttentionBlock(nn.Module):
    """
    An attention block that allows spatial positions to attend to each other (3D version).
    Originally ported from here, but adapted to the 3D case.
...
class UNet(nn.Module):
    """
    The full UNet model with attention and embedding (3D version).
    :param in_channel: channels in the input Tensor.
