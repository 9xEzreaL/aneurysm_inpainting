...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        query, key, value = qkv.chunk(3, dim=2)  # bhdzyx

        attn = torch.einsum("bncdhw, bnczyx -> bndhwyx", query, key).contiguous() / math.sqrt(channel)
        attn = attn.view(batch, n_head, depth, height, width, -1)
        attn = torch.softmax(attn, -1)
        attn = attn.view(batch, n_head, depth, height, width, depth, height, width)

        out = torch.einsum("bndhwyx, bnczyx -> bncdhw", attn, value).contiguous()
